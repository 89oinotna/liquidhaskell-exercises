...
{-@ go :: acc:List a -> xs:List a -> {v:List a | size v == size xs + size acc} @-}
reverse xs        = go [] xs
  where
    go acc []     = acc
...
\begin{code}
{-@ zipOrNull :: as:[a] 
                  -> bs:{vb:[b] | size as != 0 ==> (size bs == size as || size bs==0) }
                  -> {v:[(a,b)] | Tinier v as bs} @-}
zipOrNull       :: [a] -> [b] -> [(a, b)]
zipOrNull [] _  = []
zipOrNull _ []  = []
...
{-@ predicate Min X Y Z = (if Y < Z then X = Y else X = Z)  @-}
\begin{code}
{-@ take :: n:Nat -> as:[a] -> {v:List a | Min (size v) n (size as)} @-}
take 0 _       = []
take _ []      = []
take n (x:xs)  = x : take (n-1) xs
