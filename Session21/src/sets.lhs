...
{-@ revHelper :: acc:List a -> xs:List a -> ListUn a acc xs@-}
revHelper acc []     = acc
...
\begin{code}
{-@ halve :: _ -> xs:List a -> {v:([a], [a]) | Union (elts xs) (elts (fst v)) (elts (snd v))}@-}
halve            :: Int -> [a] -> ([a], [a])
halve 0 xs       = ([], xs)
...
\begin{code}
{-@ elem      :: (Eq a) => el:a -> xs:[a] -> {v:Bool| v <=> In el xs} @-}
elem _ []     = False
...
\begin{code}
{-@ partition :: _ -> xs:List a -> {v:([a], [a]) | Union (elts xs) (elts (fst v)) (elts (snd v))}@-}
...
\begin{code}
{-@ filter' :: _ -> xs:List a -> {v:ListSub a xs | unique xs ==> unique v} @-}
filter' _ []   = []
filter' f (x:xs)
  | f x       = x : xs'
  | otherwise = xs'
...
reverse         = go []
  where
    {-@ go     :: acc:UList a -> {xs:UList a | Disjoint (elts acc) (elts xs)}
                  -> {v:UList a | Union (elts v) (elts acc) (elts xs) && (len v == len acc + len xs)}@-}
    go acc []     = acc
...
\begin{code}
{-@ append       :: xs:UList a -> {ys:UList a | Disjoint (elts xs) (elts ys)} 
                    -> {v:UList a | Union (elts v) (elts xs) (elts ys) } @-}
append []     ys = ys
append (x:xs) ys = x : append xs ys
\end{code}
